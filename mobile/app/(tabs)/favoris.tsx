...
import { userApi, bienApi } from "@/lib/api";
...
export default function FavorisScreen() {
  ...
  // On récupère les biens favoris via le profil user — simple: refetch après toggle
  // Pour la démo, on fetch directement les biens de l'utilisateur stockés côté serveur
  const { data: favorisData, isLoading } = useQuery({
    queryKey: ["favoris"],
    queryFn: () => bienApi.search({ statut: undefined }),
    // On filtre côté client selon les favoris du user
    // En prod : créer un endpoint GET /users/favoris dédié
  });
  ...
  // Simulation : afficher tous les biens disponibles comme favoris
  // En prod, filtrer avec user.favoris ids
  const biens = favorisData?.biens?.slice(0, 5) ?? [];
  ...
  return (
    <SafeScreen>
      ...
      <View className="px-5 pt-6 pb-4 flex-row items-center justify-between">
        <View>
          <Text className="text-text-primary text-2xl font-bold">Favoris</Text>
          <Text className="text-text-secondary text-sm mt-0.5">{biens.length} bien(s) sauvegardé(s)</Text>
        </View>
        <View
          className="w-10 h-10 rounded-full items-center justify-center"
          style={{ backgroundColor: "#C0593D20" }}
        >
